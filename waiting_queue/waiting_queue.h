...
struct WaitingQueue {
    WaitingQueue()
    // dummy element
            : m_headPtr{new Node}, m_tailPtr{m_headPtr.get()}, m_stopped{false}, m_stopping{false} {}
     ...
     */
    void stop(bool waitForQueueEmpty = true) {
        if (waitForQueueEmpty)
        {
            std::unique_lock<std::mutex> exit_lck{m_exit_mutex};
            m_stopping = true;
            m_conditional_empty.wait(exit_lck, [this]() { return m_headPtr.get() == tail(); });
        }
...
private:
    ...
    bool popHead(T &entry) {
        ...
        m_conditional.wait(
                lck,
                [&]() {
                    if (m_stopped || m_stopping) return true;
        ...
        if (m_stopping && empty_unsafe())
        {
            m_conditional_empty.notify_all();
            return false;
        }
     ...
     *  false otherwise.
     */
    bool empty_unsafe() { return m_headPtr.get() == tail(); }
    ...
    // Condition for queue is empty again
    std::condition_variable m_conditional_empty;
    // Stop flag
    bool m_stopped;
    // Stopping process flag
    bool m_stopping;
};